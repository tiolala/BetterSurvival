...
execute if score @s idunico = @e[tag=working_bee,tag=axe,distance=..1.5,tag=carry_item,limit=1,sort=nearest] idunico run tag @e[tag=working_bee,tag=axe,distance=..2,tag=carry_item,limit=1,sort=nearest] add dropping_item
