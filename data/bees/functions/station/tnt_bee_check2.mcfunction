...
execute if score @s idunico = @e[tag=working_bee,tag=tnt,distance=..1.5,tag=returning,limit=1,sort=nearest] idunico run tag @e[tag=working_bee,tag=tnt,distance=..2,tag=returning,limit=1,sort=nearest] add dropping_item
