...
execute as @e[type=arrow,tag=nullarrow,nbt={inGround:0b}] if score @s idunico = @e[type=armor_stand,tag=nullarrow,limit=1,sort=nearest] idunico run tag @s add temp
...
execute unless entity @e[tag=temp] positioned ~-0.5 ~-0.5 ~-0.5 as @e[tag=mob,dx=0,dy=0,dz=0] at @s run scoreboard players remove @s wandcd 40
execute unless entity @e[tag=temp] positioned ~-0.5 ~-0.5 ~-0.5 as @a[dx=0,dy=0,dz=0] at @s run scoreboard players remove @s wandcd 40
