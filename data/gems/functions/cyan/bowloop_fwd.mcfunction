...
execute as @e[type=arrow,tag=exparrow,nbt={inGround:0b}] if score @s idunico = @e[type=armor_stand,tag=exparrow,limit=1,sort=nearest] idunico run tag @s add temp
...
#execute unless entity @e[tag=temp] positioned ~-0.5 ~-0.5 ~-0.5 as @e[tag=!exparrow,type=!armor_stand,dx=0,dy=0,dz=0,type=!player] at @s run function global:freeze_me

execute unless entity @e[tag=temp] run scoreboard players add @s aux 1
execute if entity @s[scores={aux=1..}] at @s run summon minecraft:creeper ~ ~ ~ {Silent:1b,Invulnerable:1b,ExplosionRadius:3b,Fuse:1,ignited:1b}
kill @s[scores={aux=1..}]
tag @e[tag=temp] remove temp
