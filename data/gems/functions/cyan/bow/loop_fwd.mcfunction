...
execute as @e[type=arrow,tag=bowshot,nbt={inGround:0b}] if score @s idunico = @e[type=marker,tag=bowshot,tag=cyan,limit=1,sort=nearest] idunico run tag @s add ttag01
...
execute unless entity @e[tag=ttag01] at @e[distance=..6,scores={hurttime=1..10},limit=1,sort=nearest] positioned ~ ~1 ~ run function gems:cyan/bow/boom
